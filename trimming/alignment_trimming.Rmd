...
Separate for loop solution:
```{bash, eval=F}
$ for i in $(ls *mafft); do BMGE -i $i -t AA -h 0.7 -o $i.BMGE0.7 ; done
$ for i in $(ls *mafft); do BMGE -i $i -t AA -h 0.5 -o $i.BMGE0.5 ; done
```

Nested for loop solution:
```{bash, eval=F}
$ for i in $(ls *mafft); do for j in $(echo "0.5 0.7"); do BMGE -i $i -t AA -h $j -o $i.BMGE${j} ; done ; done
```
