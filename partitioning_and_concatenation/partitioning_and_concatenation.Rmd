...
1. With your Guacamole terminal download the data set using the following URL: https://github.com/JLSteenwyk/2024_phylogenomics_workshop/blob/main/partitioning_and_concatenation_data/partitioning_and_concatenation_data.tar.gz
...
pk_create_concat -a alignments.txt -pre concat
```

7. Three output files were created.
* `r colorize("What does each file contain?", "blue")`
...
8. Certain programs require nexus formatted partition files. To prepare you for creating nexus files, modify concat.partition to be in nexus format with the following command using an "in-house script":
...
Objectives:
1. Determine the number of partitions that can describe the data using a scheme similar to jModelTest/ProtTest within IQTREE using the following command:
...
Note, this may take longer. Be patient and check on how your neighbor is doing.
...
* `r colorize("How do these results using TESTMERGEONLY and MF+MERGE?", "blue")`
...
Test model fit across the site homogenous model `LG` and `LG` with varying amino acid equilibrium frequencies across sites using the following command:

```{bash, eval=F}
iqtree2 -s concat.fa -m TESTONLY -mset LG+G4,LG+C20,LG+C60 -pre concat_partition_concat_partition_site_heterogeneous
```
...
* there is a lot of noise among single genes (this is typically true too)
